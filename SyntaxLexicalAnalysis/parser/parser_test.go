...
func test(t *testing.T, reader io.Reader, expected string) {
	t.Helper()

	wr := strings.Builder{}

	p := NewRecursiveDescentParser(reader, &wr)
	p.OutputComments = false

	p.CompileAsXml()
	if p.errorHappend {
		t.Error("error")
	}
	isOk, msg := jackhelper.CompareStringLines(wr.String(), expected)
...
func TestExpressionLess0(t *testing.T) {
	reader := strings.NewReader(`
		/** Expressionless version of
